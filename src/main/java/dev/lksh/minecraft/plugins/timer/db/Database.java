...
import dev.lksh.minecraft.plugins.timer.db.Error; // YOU MUST IMPORT THE CLASS ERROR, AND ERRORS!!!
import dev.lksh.minecraft.plugins.timer.db.Errors;
...
public abstract class Database {
  ...
  public Integer getTokens(String string) {
    ...
    try {
      ...
      rs = ps.executeQuery();
      while (rs.next()) {
        if (rs.getString("player").equalsIgnoreCase(string.toLowerCase())) { // Tell database to search for the player
                                                                             // you sent into the method. e.g
                                                                             // getTokens(sam) It will look for sam.
          return rs.getInt("kills"); // Return the players ammount of kills. If you wanted to get total (just a
                                     // random number for an example for you guys) You would change this to total!
        }
      }
    } catch (SQLException ex) {
  ...
  public void setTokens(Player player, Integer tokens, Integer total) {
    ...
    try {
      conn = getSQLConnection();
      ps = conn.prepareStatement("REPLACE INTO " + table + " (player,kills,total) VALUES(?,?,?)"); // IMPORTANT. In
                                                                                                   // SQLite class, We
                                                                                                   // made 3 colums.
                                                                                                   // player, Kills,
                                                                                                   // Total.
      ps.setString(1, player.getName().toLowerCase()); // YOU MUST put these into this line!! And depending on how many
                                                       // colums you put (say you made 5) All 5 need to be in the
                                                       // brackets
                                                       // Seperated with comma's (,) AND there needs to be the same
                                                       // amount of
                                                       // question marks in the VALUES brackets. Right now i only have 3
                                                       // colums
                                                       // So VALUES (?,?,?) If you had 5 colums VALUES(?,?,?,?,?)

      ps.setInt(2, tokens); // This sets the value in the database. The colums go in order. Player is ID 1,
