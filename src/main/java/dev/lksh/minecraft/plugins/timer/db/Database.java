...
public abstract class Database {
  ...
  public void initialize() {
    ...
    try {
      PreparedStatement ps =
          connection.prepareStatement("SELECT * FROM " + eventsTable + " WHERE event_name = ?");
  ...
  public void createEvent(TimerEvent event) {
    ...
    try {
      ...
      ps =
          conn.prepareStatement(
              "INSERT INTO events (event_id, event_name, start_x, start_y, start_z, start_margin,"
                  + " end_x, end_y, end_z, end_margin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      ps.setString(1, event.eventId);
      ps.setString(2, event.eventName);
      ps.setInt(3, event.startX);
      ps.setInt(4, event.startY);
      ps.setInt(5, event.startZ);
      ps.setInt(6, event.startMargin);
      ps.setInt(7, event.endX);
      ps.setInt(8, event.endY);
      ps.setInt(9, event.endZ);
      ps.setInt(10, event.endMargin);
  ...
  //    rs = ps.executeQuery();
  //    while (rs.next()) {
  //      if (rs.getString("player").equalsIgnoreCase(string.toLowerCase())) { // Tell database to
  // search for the player
  //                                                                           // you sent into the
  // method. e.g
  //                                                                           // getTokens(sam) It
  // will look for sam.
  //        return rs.getInt("kills"); // Return the players ammount of kills. If you wanted to get
  // total (just a
  ...
  //
  // // Total.
  //    ps.setString(1, player.getName().toLowerCase()); // YOU MUST put these into this line!! And
  ...
  // VALUES(?,?,?,?,?)
  //
  //    ps.setInt(2, tokens); // This sets the value in the database. The colums go in order. Player
  // is ID 1,
